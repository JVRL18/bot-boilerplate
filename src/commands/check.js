...
return fetch(`https://uniteapi.dev/p/${player}`)
.then( async r => {
    let a = await (await r.text())
    let innerText = 'meta content="Pokémon Unite :'
    let enderText = 'property="og:description"/'
    let expectedText = a.slice(a.indexOf(innerText) + innerText.length + 1, a.indexOf(enderText) - 2)
    const formatter = (update) => {
        const rankFormatter = function(e){
            ...
            if(name.includes('Master') === true){
                ...
                infos.rank = infos.rank.slice(0,infos.rank.indexOf('null'))+` ${Number(rankPoints)}`
        ...
        let wholeRank = rankFormatter(edit.slice(rankFinder, edit.indexOf(battlesFinder)))

        let battles = Number(edit.slice(edit.indexOf(battlesFinder) + battlesFinder.length, edit.indexOf(winsFinder)))

        let wins = Number(edit.slice(edit.indexOf(winsFinder) + winsFinder.length, edit.indexOf(wrFinder)))

        let winRate = Number(edit.slice(edit.indexOf(wrFinder) + wrFinder.length))

        finalData.level = Number(edit.slice(lvFinder.length, lvFinder.length + 2))
...
module.exports = {
    ...
    .setName("check")
    .setDescription("Get pokemon unite info about choosen user").addUserOption(
        option => option
        .setName("user")
    ...
    run: (interaction,client,typo) => {
        ...
        interaction.reply({embeds:[embed_404_error_message(`This command is actually set to be only text, try again using \`${client.prefix}check <user>\` `)]})
    },
